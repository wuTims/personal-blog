...
const accentClasses = {
  emerald: {
    ...
    bg: 'bg-emerald/5 dark:bg-emerald/10',
    text: 'text-emerald',
    glow: 'shadow-[0_0_30px_-5px] shadow-emerald/20 dark:shadow-emerald/30',
  },
  coral: {
    border: 'border-coral/30',
    bg: 'bg-coral/5 dark:bg-coral/10',
    text: 'text-coral',
    glow: 'shadow-[0_0_30px_-5px] shadow-coral/20 dark:shadow-coral/30',
  },
  lavender: {
    border: 'border-lavender/30',
    bg: 'bg-lavender/5 dark:bg-lavender/10',
    text: 'text-lavender',
    glow: 'shadow-[0_0_30px_-5px] shadow-lavender/20 dark:shadow-lavender/30',
  },
  sky: {
    border: 'border-sky/30',
    bg: 'bg-sky/5 dark:bg-sky/10',
    text: 'text-sky',
    glow: 'shadow-[0_0_30px_-5px] shadow-sky/20 dark:shadow-sky/30',
  },
}
  ...
  return (
    <Container className="py-20 sm:py-32">
      <div className="flex flex-col items-center justify-center text-center">
        {/* WIP Badge */}
        <div
          className={`mb-8 inline-flex items-center gap-2 rounded-full border px-4 py-1.5 ${accent.border} ${accent.bg}`}
        >
          <span className={`h-2 w-2 animate-pulse rounded-full ${accent.text} bg-current`} />
          <span className={`text-sm font-medium ${accent.text}`}>Work in Progress</span>
        </div>
        ...
        {/* Decorative Element */}
        <div className={`mt-12 rounded-lg border ${accent.border} ${accent.bg} ${accent.glow} p-8`}>
          <div className="flex flex-col items-center gap-4">
            <RobotIcon className={`size-16 ${accent.text}`} />
          </div>
        </div>
        ...
        <a
          href="/"
          className="mt-10 text-sm font-medium text-neutral-600 transition-colors hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-neutral-100"
        >
